//Kinematics Challenge 3: PID Control Test
//Robot max move linear velocity=0.025m/s
//Robot max rotate angular velocity=36.00 degrees/s=0.2*pi/s
...
double KP=10.00,KD=0.025,KI=5.00;
double G;
...
Dest_vel cmd_vel(){
  ...
  double alpha_dest,theta_dest,dest_rotation_t;
  ...
  //Can change this to global value
  dest_vel.dest_V=0.025;
  dest_vel.dest_W=0.2*pi;

  dest_rotation_t=theta_dest/dest_vel.dest_W;

  //To be added: assert the signal of theta_dest and rotate for indicated amount of time
...
void setup() {
  ...
  pose_dest=createPose(5.00,8.00,90); //use a random destination pose for test

  Serial.begin(9600);

  attachInterrupt(digitalPinToInterrupt(3),encFunc,CHANGE);
  
  dest_vel=cmd_vel(); //1. Give the robot a desired linear and angular velocities
  ...
  w_wheels_dest=cmd_vel2wheels(dest_vel.dest_V,dest_vel.dest_W); //2. Calculate the desired angular velocities of the wheels,
...
void loop() {
  if(millis()%t==0){
    pose_orig=poseUpdate(N_L,N_R,r,b,C); //3. Calculate the real angular velocities of the wheels and update robot pose

    printRobotPose();
    
    G=pid_controller(w_wheels_dest,w_wheels);
